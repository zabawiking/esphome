...
void OpenthermHub::handle_timeout_error_() {
  ...
  for (MessageId &msg : this->messages_) {
    ESP_LOGD(TAG, "Set NAN value for data id %d", msg);
    switch (msg) {
      OPENTHERM_SENSOR_MESSAGE_HANDLERS(OPENTHERM_MESSAGE_RESPONSE_MESSAGE, OPENTHERM_MESSAGE_TIMEOUT_SENSOR_ENTITY, ,
                                        OPENTHERM_MESSAGE_RESPONSE_POSTSCRIPT, )
    }
    switch (msg) {
      OPENTHERM_BINARY_SENSOR_MESSAGE_HANDLERS(OPENTHERM_MESSAGE_RESPONSE_MESSAGE, OPENTHERM_MESSAGE_TIMEOUT_BINARY_SENSOR_ENTITY, ,
                                              OPENTHERM_MESSAGE_RESPONSE_POSTSCRIPT, )
    }
  }
  this->stop_opentherm_();
